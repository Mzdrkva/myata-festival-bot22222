...
# Для обратного преобразования ("мая" → 5)
...
# ====== Загрузка/сохранение сцен и избранного ======
# Если файла ещё нет, load_json создаст его с пустым default
DEFAULT_SCENES = {
    # Пример: сцена Test с одним тестовым артистом 31 мая
    "Test": [
...
def date_menu_kb():
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    # Динамически вычисляем "сегодняшнюю" дату в формате "D месяц"
    now = datetime.now()
    day = now.day

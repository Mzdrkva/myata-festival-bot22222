...
DEFAULT_SCENES = {
    ...
    "Маяк": [
        ...
        ["2025-06-15 13:40", "Каспий"],  # поменяли: раньше Ник Брусковский
        ["2025-06-15 15:40", "Ник брусковский"],  # поменяли: раньше Каспий
...
FAQ_TEXTS = {
    ...
    "Обмен билетов": (
        ...
        "- Убедитесь, пожалуйста, что все топоры/лопаты/вилы/рапиры/ром/виски/кола и т.д. остались дома. Парковка — это тоже территория фестиваля, на которую распространяется список запрещенных к провозу предметов.\n"
        "Провоз запрещенных вещей ведет к увеличению времени досмотра, неприятным моментам для вас, для службы безопасности и для других гостей.\n"
        "- При заезде на парковку приготовьте, пожалуйста, билет на парковку. Вам будет выдана наклейка на лобовое стекло, с которой вы сможете при необходимости выехать и заехать на парковку вновь, с указанием номера автомобиля. Обратите внимание, что наклейка выдается один раз на всё время фестиваля, не срывайте её при выезде. Иначе вы не сможете вернуться на парковку.\n"
        "- После того, как вы припаркуетесь, приготовьте ваши входные билеты и проходите к входной группе со стороны парковки. Удобно идти сразу с вещами, чтобы пройти досмотр и войти на фестиваль.\n\n"
        "Мы понимаем, что не все приезжают с большим количеством вещей, поэтому разделяем потоки: если у вас объемные рюкзаки, которые необходимо пропускать через рентген — пройдите по треку «с багажом». Если у вас нет вещей или небольшая поясная сумка, если вы с детьми — для вас будет отдельный трек.\n\n"
        "• Если вы приезжаете на трансфере/такси/пешком:\n"
        "- Рекомендуем пользоваться Главной входной группой — со стороны Инфоцентра фестиваля. Приготовьте ваши билеты и личные вещи к досмотру, и пройдите в соответствующем направлении: если у вас объемные рюкзаки для рентгена — пройдите по треку «с багажом». Если у вас нет вещей (или только поясная сумка) или вы с детьми — для вас будет отдельный трек.\n\n"
        "• Если вы приехали ночью, то получить браслет можно будет на въезде на парковку вместе с парковочной наклейкой.\n\n"
        "• Билеты категории «Место под палатку» меняются вместе с входными билетами. Вам будет выдана наклейка с вашим рядом и местом, которую нужно будет обязательно прикрепить на растяжку палатки.\n"
        "После прохождения на фестиваль, если вы впервые в кемпинге и не знаете, где ваш палаточный лагерь — найдите волонтеров, которые стоят за турникетами и помогут сориентироваться."
    ),
    ...
    "Развлечения на фестивале": (
        ...
        "• Спорт-площадки.\n"
        "• Beauty zone: ваши фестивальные образы, мэйк-ап, блестки, барбершоп, де-дреды и т.д.:\n"
    ...
    "Правила фестиваля": (
        ...
        "список разрешенных к провозу продуктов, другие правила и ограничения на фестивале, можно узнать в разделе "
    ...
    "Расписание выступлений, карта фестиваля": (
        "Информация о выступлениях по дням появится примерно за 2−4 недели до старта фестиваля — "
    ...
    "Инфоцентр": (
        ...
        "или дождаться отправления автобуса. Команда фестиваля всегда готова помочь с любыми вопросами."
    ),
    "Социальные скидки": (
        ...
        "Скидка распространяется на все категории билетов (не более одного билета каждой категории на человека). Обязательно нужны оригиналы документов."
    ),
    "Дети до 6-ти лет — нужен ли билет?": (
        "Дети до 6-ти лет проходят на фестиваль бесплатно с родителями (законными представителями), билет не нужен.\n\n"
        "На фестивале открыта комната матери и ребенка со всем необходимым для разогрева детского питания, обеда, стирки и сушки одежды — пеленальные столики, душ с горячей водой и раковины. Доступ свободный.\n\n"
    ...
    "Парковка": (
        ...
        "«Место на парковке для крупногабаритного транспорта (автодома)» можно на <a href='https://mintmusic.ru/parking'>сайте фестиваля</a>.\n\n"
        "При въезде билет обменивается на наклейку, которую необходимо разместить на лобовом стекле вашего автомобиля. "
        "Эта наклейка позволяет выезжать и заезжать на парковку неограниченное число раз все дни фестиваля.\n\n"
        "На парковке круглосуточно будут дежурить пикапы от «Т-Страхования», которые помогут бесплатно накачать колеса, зарядить аккумулятор, подвезти топливо и правильно оформить ДТП в случае необходимости.\n\n"
        "Установка палаток на территории парковки запрещена! (Туристические, кемпинговые, дорожные, портативные душевые, туалеты и мини-туалеты не разрешены в кемпингах и на парковке.)"
    ),
    ...
    "Удобства на фестивале": (
        "Фестиваль «Фестиваль 2025» — один из самых комфортных в России.\n"
        "На территории кемпинга вы найдете ресепшн, бесплатные душевые, уборные, умывальники, питьевые фонтанчики, места для зарядки гаджетов и многое другое."
    ),
    "Продукты питания на фестивале": (
        "На фестивале будет более 50 ресторанов и фудтраков, включая вегетарианские кафе. Следите за обновлениями на сайте и в соцсетях, чтобы узнать участников нашего фудкорта.\n\n"
        ...
        "– Любые напитки, в том числе алкогольные, в любой таре (так как на фестивале доступны питьевые фонтанчики);\n"
        "– Скоропортящиеся продукты и продукты в опасной упаковке;\n"
        "– Консервы в металлических или стеклянных банках, полуфабрикаты, варёные колбасы, маринованное, варено-копченое мясо, рыба и т.п."
    ),
    "Кипяток на фестивале": (
        "На территории фестиваля вам будет доступен бесплатный кипяток от нашего водного партнера, компании «Барьер». Вы найдете места с водой от «Барьера» в буклете фестиваля.\n\n"
        "График работы точки кипятка:\n"
        "Четверг: 19:00 - 22:00\n"
        "Пятница | Суббота | Воскресенье: 8:00 - 11:00, 13:00 - 15:00, 19:00 - 22:00"
    ),
    "Костры и газовые горелки": (
        "Мы очень ответственно относимся к пожарной безопасности. Разведение костров и пронос воспламеняющихся устройств (включая палаточные кемпинги и парковку) запрещены.\n"
        "Газовые горелки на территории опен-эйров запрещены законодательством РФ."
    ),
    "Первая помощь и медикаменты": (
        "Как и в предыдущие годы, на фестивале будут организованы пункты первой помощи и дежурная скорая помощь.\n\n"
        "Средства первой помощи (анальгин, активированный уголь и т.п.) допускаются в оригинальной упаковке, остальные лекарства — только по рецепту."
    ),
    "Курение и алкоголь": (
        "На территории фестиваля запрещено курение и распитие алкогольных напитков, за исключением специально отведённых мест с ограничением 18+."
    )
}

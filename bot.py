...
WELCOME_IMAGE = "welcome.jpg"  # Положите файл welcome.jpg рядом с bot.py
                ...
                if not e["notified"]:
                    ...
                    if 0 < delta <= 15*60:
                        await bot.send_message(
                            chat_id=int(uid),
                            text=(
                                f"🔔 Через 15 минут: {e['artist']} "
                                f"({e['scene']}) в {e['time'][11:16]}"
                            )
                        )
                        e["notified"] = True
                        updated = True
...
async def cmd_start(msg: types.Message):
    ...
    # отправляем фото вместе с подписью
...
async def cmd_choose_date(msg: types.Message):
    ...
    for idx, (tstr, artist) in enumerate(entries):
        kb.add(InlineKeyboardButton(f"{tstr[11:16]} — {artist}",
                   callback_data=f"fav|{scene}|{iso_date}|{idx}"))

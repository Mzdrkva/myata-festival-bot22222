...
FAQ_TEXTS = {
    ...
    "Место на парковке": (
        "Парковка работает с 12 июня 18:00 до 15 июня 18:00 без перерывов.\n"
        "Стойки парковки расположены у главного входа.\n"
        "Камеры хранения для машин находятся рядом с пропускным пунктом."
    ),
    ...
    "Карта фестиваля": None,  # Обработаем отдельно (отправка картинки)
...
async def info_map(msg: types.Message):
    # Отправляем изображение карты (файл map.jpg должен быть рядом с bot.py)

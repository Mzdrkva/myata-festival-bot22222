...
main_kb.row(KeyboardButton("FAQ"), KeyboardButton("Расписание сцен"), KeyboardButton("⭐ Избранное"))

schedule_kb = ReplyKeyboardMarkup(resize_keyboard=True)
schedule_kb.row(
    KeyboardButton("SIRENA"), KeyboardButton("TITANA"),
    KeyboardButton("Сцена 3"), KeyboardButton("Сцена 4")
)
schedule_kb.row(
    KeyboardButton("Сцена 5"), KeyboardButton("Сцена 6"),
    KeyboardButton("Сцена 7"), KeyboardButton("◀️ Назад")
)

date_kb = ReplyKeyboardMarkup(resize_keyboard=True)
date_kb.row(KeyboardButton("13 июня"), KeyboardButton("14 июня"), KeyboardButton("15 июня"))
...
for i in range(0, len(faq_options), 2):
    ...
    faq_kb.row(*(KeyboardButton(text) for text in row))
faq_kb.row(KeyboardButton("◀️ Назад"))

# ====== Фильтрация расписания по дате (с учётом "ночи") ======
...
async def handle_star(callback: types.CallbackQuery):
    _, scene, iso_date, idx = callback.data.split("|")
    idx = int(idx)
    entries = get_entries_for_date(scene, iso_date)
...
async def handle_faq(message: types.Message):
    topic = message.text
    # Здесь можно подставить реальный текст для каждой темы
    answers = {
        "О фестивале": "«Мята 2025» — это трёхдневный фестиваль...",
        "Расписание сцен": "Выберите «Расписание сцен» в главном меню.",
        "Обмен билетов на браслеты": "Билеты обмениваются у входа...",
        "Место на парковке": "Парковка находится по адресу...",
        "Место под палатку": "Зона кемпинга рядом с лесом...",
        "Карта Фестиваля": "Скачать карту: https://example.com/map.png",
        "Расписание работы душевых": "Душевые открыты с 8:00 до 22:00...",
        "Расписание зон с кипятком": "Кипяток выдают в палатках 5 и 9...",
        "Трансферы": "Трансферы от ж/д станции каждые 30 мин...",
        "Расписание электричек": "Электрички отправляются в 7:45 и 18:20..."
    }
    await message.reply(answers.get(topic, "Информация скоро появится."), reply_markup=faq_kb)
